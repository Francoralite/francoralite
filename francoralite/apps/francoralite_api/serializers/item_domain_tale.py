...
class ItemDomainTaleSerializer(serializers.ModelSerializer):
    """
    Common serializer for all ItemDomainTale actions
    """

    # fields of the serializer
    item = AsymetricRelatedField.from_serializer(
        ItemSerializer, kwargs={'required': True})
    domain_tale = AsymetricRelatedField.from_serializer(
        DomainTaleSerializer, kwargs={'required': True})

    class Meta:
        model = ItemDomainTaleModel
        fields = '__all__'
    ...
    #
    #     # Create an oject item_domain_tales
    #     item_domain_tales = \
    #         ItemDomainTaleModel.objects.create(
    #             collection=collection, collector=collector, **validated_data)
    #
    #     return item_domain_tales
