...
class ItemUsefulnessSerializer(serializers.ModelSerializer):
    """
    ...
    usefulness = AsymetricRelatedField.from_serializer(
        UsefulnessSerializer, kwargs={'required': True})

    class Meta:
        model = ItemUsefulnessModel
        fields = '__all__'
    ...
    #
    #     # Create an oject item_usefulnesses
    #     item_usefulnesses = \
    #         ItemUsefulnessModel.objects.create(
    #             collection=collection, collector=collector, **validated_data)
    #
    #     return item_usefulnesses
