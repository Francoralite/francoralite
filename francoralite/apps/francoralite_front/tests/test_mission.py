...
def test_mission_list(francoralite_context):
    for username in francoralite_context.USERNAMES:
        ...
        assert has_buttons == francoralite_context.exists_element(
            by_link_url='/mission/edit/1')
        assert has_buttons == francoralite_context.exists_element(
            by_link_url='/mission/edit/2')
        assert has_buttons == francoralite_context.exists_element(
            by_button_url='/mission/delete/1')
        assert has_buttons == francoralite_context.exists_element(
            by_button_url='/mission/delete/2')
...
def test_mission_add(francoralite_context):
    for username in francoralite_context.WRITERS:
        ...
        francoralite_context.verify_title(
            _("Fonds : Fonds issus des Archives de Folklore et d'Ethnologie [Exemple]"))

        # Click on the "add" link
        francoralite_context.find_element(
            by_link_url='/institution/1/fond/1/mission/add').click()
...
def test_mission_409_err(francoralite_context):
    for username in francoralite_context.WRITERS:
        # Go to the home page
        francoralite_context.open_homepage(auth_username=username)
        ...
        # Write a code
        francoralite_context.set_element_value('id_code', 'UPOI_ATP_0000')
