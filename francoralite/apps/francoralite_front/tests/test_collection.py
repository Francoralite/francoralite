...
def test_collection_add(francoralite_context):
    ...
    francoralite_context.find_element(
        by_link_url="/institution/1/fond/1/mission/1/collection/add"
    ).click()
...
def test_collection_update(francoralite_context):
    for username in francoralite_context.WRITERS:
        ...
        # Add an item
        francoralite_context.find_element(by_id="btn_edit_collection").click()

        # Verify the label of the collection page
        francoralite_context.verify_title(_("Enquêtes - Modification"))
...
def test_collection_add_item(francoralite_context):
    """
    A function that adds an item to a collection in the francoralite context.
    Bug : issue #468
    Args:
        francoralite_context: The context object for the francoralite application.
    ...
    for username in francoralite_context.WRITERS:
        ...
        # Add an item
        francoralite_context.find_element(by_id="btn_add_item").click()

        # Verify the label of the item page
        francoralite_context.verify_title(_("Item Création"))
